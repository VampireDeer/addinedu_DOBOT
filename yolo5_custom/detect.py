...
def run_yolo_detection(weights, source, data, imgsz, conf_thres, iou_thres, max_det, device, view_img, save_txt,
    ...
    capture_frame_number = 100 # 캡쳐하고 싶은 프레임 번호 
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if frame == capture_frame_number:
                    capture_file = os.path.join("/home/seokwon/dev_ws/dobot/project/image_files/", "image.png")
                    cv2.imwrite(capture_file, im0)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #result_xy_values.append([names[int(cls)], xyxy[0], xyxy[1], xyxy[2], xyxy[3]])
                    result_xy_values.append([names[int(cls)], (xyxy[0] + xyxy[2])/2, (xyxy[1] + xyxy[3])/2])
            # Stream results
